classdef GaussianNoiseGeneratorBandpass < symphonyui.core.StimulusGenerator
    ...
    properties
        ...
        freqCutoffHigh      % Noise frequency cutoff for smoothing (Hz), max freq
        freqCutoffLow       % Noise frequency cutoff for smoothing (Hz), min freq
    ...
    methods
        ...
        function obj = GaussianNoiseGeneratorBandpass(map)
            if nargin < 1
                map = containers.Map();
            end
    ...
    methods (Access = protected)
        ...
        function s = generateStimulus(obj)
            ...
            if mod(stimPts, 2) == 0
                ...
                oneSidedFilterLow = 1 ./ (1 + (frequencies / obj.freqCutoffHigh) .^ (2 * obj.numFilters)); % lowpass
                oneSidedFilterHigh = abs(frequencies / obj.freqCutoffLow) .^ (2 * obj.numFilters) ./ (1 + (frequencies / obj.freqCutoffLow) .^ (2 * obj.numFilters)); % highpass
                oneSidedFilter = oneSidedFilterLow .* oneSidedFilterHigh;
